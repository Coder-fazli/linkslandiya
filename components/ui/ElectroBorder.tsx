...
export const ElectroBorder: React.FC<ElectroBorderProps> = ({
  ...
  const updateFilter = () => {
    ...
    const dxAnimations = Array.from(svg.querySelectorAll<SVGAnimateElement>('feOffset > animate[attributeName="dx"]'));
    const dyAnimations = Array.from(svg.querySelectorAll<SVGAnimateElement>('feOffset > animate[attributeName="dy"]'));
    ...
    requestAnimationFrame(() => {
      [...dxAnimations, ...dyAnimations].forEach((a: SVGAnimateElement & { beginElement?: () => void }) => {
  ...
  const borderBase: CSSProperties = { border: `${borderWidth}px solid ${borderColor}`, ...radiusStyle };
  const glowLayer1: CSSProperties = effects && glow ? { ...radiusStyle, border: `${borderWidth}px solid ${toRGBA(borderColor, 0.7)}`, filter: `blur(${borderWidth * 1.2}px)`, opacity: 0.1 } : {};
  const glowLayer2: CSSProperties = effects && glow ? { ...radiusStyle, border: `${borderWidth}px solid ${toRGBA(borderColor, 0.9)}`, filter: `blur(${glowBlur}px)`, opacity: 0.5 } : {};
